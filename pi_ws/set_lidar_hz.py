...
def set_hz():
    ...
    if ser.is_open == False:
        ser.open()
    print("wait to change hz")
    time.sleep(0.5)
    print("change hz", "sucess: ")
    # print(sendCommand(SET_FRAME_RATE, FRAME_1000))
    print(sendCommand(SET_FRAME_RATE, FRAME_500))
    print("done")
    time.sleep(0.5)
    print("safe done")
...
""" def begin(port, rate):
    ...
    if ser.inWaiting() > 0:  #  If data present...
...
def getData():
    ...
    while ser.inWaiting() > TFMP_FRAME_SIZE:
        ser.read()
...
def sendCommand(cmnd, param):
    ...
    if cmnd == SET_FRAME_RATE:  #  If the command is Set FrameRate...
        cmndData[3:2] = param.to_bytes(
            2, byteorder="little"
        )  #  add the 2 byte FrameRate parameter.
    elif cmnd == SET_BAUD_RATE:  #  If the command is Set BaudRate...
        cmndData[3:3] = param.to_bytes(
            3, byteorder="little"
        )  #  add the 3 byte BaudRate parameter.
    ...
    ser.write(cmndData)  #  send command data
    ...
    if cmnd == GET_FIRMWARE_VERSION:
        ...
        if cmnd == SOFT_RESET or cmnd == HARD_RESET or cmnd == SAVE_SETTINGS:
...
def printStatus():
    ...
    if status == TFMP_READY:
        print("READY", end="")
    elif status == TFMP_SERIAL:
        print("SERIAL", end="")
    elif status == TFMP_HEADER:
        print("HEADER", end="")
    elif status == TFMP_CHECKSUM:
        print("CHECKSUM", end="")
    elif status == TFMP_TIMEOUT:
        print("TIMEOUT", end="")
    elif status == TFMP_PASS:
        print("PASS", end="")
    elif status == TFMP_FAIL:
        print("FAIL", end="")
    elif status == TFMP_I2CREAD:
        print("I2C-READ", end="")
    elif status == TFMP_I2CWRITE:
        print("I2C-WRITE", end="")
    elif status == TFMP_I2CLENGTH:
        print("I2C-LENGTH", end="")
    elif status == TFMP_WEAK:
        print("Signal weak", end="")
    elif status == TFMP_STRONG:
        print("Signal saturation", end="")
    elif status == TFMP_FLOOD:
...
if __name__ == "__main__":
    print(
        "tfmplus - This Python module supports"
